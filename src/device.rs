...
pub const REGISTERS_PROTO_SIZE: usize =
    REGISTERS_SIZE_BYTES + spi_proto::PROTOCOL_HEADER_SIZE + spi_proto::PROTOCOL_CRC_SIZE;
...
pub const DEVICE_MOTOR_BLOCK_SIZE_BYTES_READ: usize =
    DEVICE_MOTOR_BLOCK_COUNT_READ * DEVICE_REG_SIZE_BYTES; // Size of each motor block in bytes
pub const DEVICE_ALL_MOTORS_BLOCK_SIZE_BYTES_READ: usize =
    DEVICE_SUPPORTED_MOTORS * DEVICE_MOTOR_BLOCK_SIZE_BYTES_READ;
#[allow(dead_code)]
pub const DEVICE_MOTOR_BLOCK_SIZE_BYTES_WRITE: usize =
    DEVICE_MOTOR_BLOCK_COUNT_WRITE * DEVICE_REG_SIZE_BYTES; // Size of each writable motor block in bytes
#[allow(dead_code)]
...
pub const INTERNAL_LOOP_TIME_OFFSET: usize =
    MOTORS_DATA_START_OFFSET + DEVICE_ALL_MOTORS_BLOCK_SIZE_BYTES_READ;
...
impl Device {
    ...
    pub fn req_regs_dump(&mut self, reg_id: RegisterID, reg_dump: &mut [u8]) {
        ...
        debug!("Register dump requested.");
        debug!("reg dump len: {}", reg_dump.len());
        let write_buff_size =
            reg_dump.len() + spi_proto::PROTOCOL_HEADER_SIZE + spi_proto::PROTOCOL_CRC_SIZE;
        ...
        let is_valid_crc = spi_proto::validate_crc(
            &proto_buff[spi_proto::PROTOCOL_HEADER_SIZE..recv_crc_start_idx],
            recv_crc,
        );
        if is_valid_crc {
            ...
            error!("CRC validation failed received.");
        }
    }
    ...
    pub fn req_reg_write(&mut self, reg_id: RegisterID, regs: &[u8]) {
        ...
        debug!(
            "Writing to register ID: {:?}, data length: {}",
            reg_id,
            regs.len()
        );
        let mut proto_buff: [u8; REGISTERS_PROTO_SIZE] = [0; REGISTERS_PROTO_SIZE];
        let proto_buff_size_used =
            spi_proto::PROTOCOL_HEADER_SIZE + regs.len() + spi_proto::PROTOCOL_CRC_SIZE;
        ...
        spi_proto::execute_spi_transaction(&mut self.spi_dev, &mut proto_buff_used);
        info!(
            "Resiter id {:?} write request sent, error code: {:?}",
            reg_id,
            self.get_last_error_status()
        );
    }
    ...
    pub fn get_all_registers(&mut self) -> DeviceFullInfo {
        ...
        let motors_data = &reg_dump[MOTORS_DATA_START_OFFSET
            ..MOTORS_DATA_START_OFFSET + DEVICE_ALL_MOTORS_BLOCK_SIZE_BYTES_READ];
    ...
    pub fn get_motor_dump(&mut self, motor_num: MotorID) -> MotorStatus {
        let reg_id =
            RegisterID::from_motor_id(motor_num, device::MotorRegisterOffset::OperationMode);
        let mut reg_dump: [u8; DEVICE_MOTOR_BLOCK_SIZE_BYTES_READ] =
            [0; DEVICE_MOTOR_BLOCK_SIZE_BYTES_READ];
    ...
    pub fn set_motor_cfg(&mut self, motor_num: MotorID, motor_cfg: &MotorCfg) {
        let mut regs_buff: [u8; DEVICE_MOTOR_BLOCK_COUNT_WRITE] =
            [0; DEVICE_MOTOR_BLOCK_COUNT_WRITE];
        let mut offset = 0;

        regs_buff[offset..offset + DEVICE_REG_SIZE_BYTES]
            .copy_from_slice(&(motor_cfg.mode as u32).to_le_bytes());
        offset += DEVICE_REG_SIZE_BYTES;

        regs_buff[offset..offset + DEVICE_REG_SIZE_BYTES]
            .copy_from_slice(&(motor_cfg.direction as u32).to_le_bytes());
        offset += DEVICE_REG_SIZE_BYTES;
    ...
    pub fn get_motor_dump_all(&mut self) -> [MotorStatus; DEVICE_SUPPORTED_MOTORS] {
        ...
        let mut reg_dump: [u8; DEVICE_ALL_MOTORS_BLOCK_SIZE_BYTES_READ] =
            [0; DEVICE_ALL_MOTORS_BLOCK_SIZE_BYTES_READ];
    ...
    pub fn print_full_device_info(dev_info: &DeviceFullInfo) {
        info!("Device full info:");
        info!("----------------");
        info!("Device ID: {}", dev_info.system_info.device_id);
        info!(
            "Firmware Version: {}",
            dev_info.system_info.firmware_version
        );
        info!("Internal Loop Time: {}", dev_info.internal_loop_time_ms);
        info!("Last Error Status: {:?}", dev_info.last_error_status);
        info!("----------------");
        for (idx, status) in dev_info.motors_status.iter().enumerate() {
            let motor_num = idx + 1;
            info!("Motor {} Status:", motor_num);
            info!("  [{}]Control Mode: {:?}", motor_num, status.mode);
            info!("  [{}]Direction: {:?}", motor_num, status.direction);
            info!("  [{}]PWM Duty Cycle: {}", motor_num, status.pwm_duty_cycle);
            info!(
                "  [{}]Counts per Revolution: {}",
                motor_num, status.counts_per_revolution
            );
            info!("  [{}]RPM Current: {}", motor_num, status.rpm_current);
            info!("  [{}]RPM Desired: {}", motor_num, status.rpm_desired);
            info!(
                "  [{}]PID Params: Kp={}, Ki={}, Kd={}",
